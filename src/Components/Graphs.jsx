...
const Graphs = (props) => {
    ...
    useEffect(() => {
        ...
        async function fetchTOPUserLikedContent() {
            const response = await fetch(`${Server_Url}User/TOPUserLikedContent/${props.match.params.UserName}`)
            const result = await response.json()
            console.log('TOPUserLikedContent', result)
            const NewData = Data;
            const ContentNames = result.map(content => content.ContentName)
            const Likes = result.map(content => parseInt(content.Likes))
            NewData.graphset[0].scaleX.values = ContentNames
            NewData.graphset[0].series[0].values = Likes
